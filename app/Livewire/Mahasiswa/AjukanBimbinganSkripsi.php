...
use App\Models\JadwalBimbingan; // Pastikan nama model dan path-nya benar
use App\Models\User;
use Carbon\Carbon;
use DateInterval;
use DatePeriod;
...
class AjukanBimbinganSkripsi extends Component
{
    use WithFileUploads;

    public Collection $pembimbingTersedia;
    public $dosen_id = '';
    public $jadwal = '';
    public $judul = '';
    public $topik = '';
    ...
    public function mount()
    {
        ...
        if ($mahasiswa->pembimbingSkripsi1) {
            $pembimbing[] = $mahasiswa->pembimbingSkripsi1;
        }

        if ($mahasiswa->pembimbingSkripsi2) {
            $pembimbing[] = $mahasiswa->pembimbingSkripsi2;
        }

        $this->pembimbingTersedia = collect($pembimbing);
    }

    public function updatedDosenId($dosenId)
    {
        $this->jadwal = ''; 
        $this->opsiDropdown = []; 
    ...
    public function store()
    {
        ...
        try {
            ...
            Log::error('Gagal membuat ajuan bimbingan skripsi: ' . $e->getMessage());
    ...
    {
        return view('livewire.mahasiswa.ajukan-bimbingan-skripsi');
    }
}